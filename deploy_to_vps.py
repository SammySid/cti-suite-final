...
deploy_to_vps.py
================
Deploys ONLY the cti_dashboard/ sub-folder to the VPS via SFTP.
...
DASHBOARD_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cti_dashboard")

# Files / dirs inside cti_dashboard that should NOT be uploaded
EXCLUDE_DIRS  = {".git", "__pycache__", "docs"}   # docs = internal reference only
EXCLUDE_FILES = {
...
def deploy():
    ...
    if not os.path.isdir(DASHBOARD_DIR):
        raise FileNotFoundError(f"Dashboard source not found: {DASHBOARD_DIR}")

    print(f"\n🚀  CTI Dashboard — deploying to {user}@{host}:{remote_root}")
